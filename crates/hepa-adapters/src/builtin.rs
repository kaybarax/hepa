...
pub fn builtin_adapter_spec(id: &str) -> HepaAdapterSpec {
    match id {
        "fake" => spec(AdapterSpecTemplate {
            ...
            capabilities: vec!["design", "docs", "test", "review"],
        ...
        "pi" => HepaAdapterSpec {
            ...
            roles: vec![HepaAdapterRole::Worker, HepaAdapterRole::Reviewer],
            mode: HepaAdapterMode::Oneshot,
            command: "pi --no-approve --no-session --no-extensions --no-skills --no-prompt-templates --no-context-files --tools read,edit,write,bash,grep,find,ls -p --mode json --provider <provider> --model <model>".to_string(),
            review_command: Some(
                "pi --no-approve --no-session --no-extensions --no-skills --no-prompt-templates --no-context-files --tools read,edit,write,bash,grep,find,ls -p --mode json --provider <provider> --model <model>".to_string(),
            ),
            workdir: "{worktree}".to_string(),
            required_commands: vec!["pi".to_string()],
            required_env: vec!["HEPA_PI_MODEL".to_string()],
            sandbox: HepaAdapterSandbox::None,
        ...
        "shell-command" => spec(AdapterSpecTemplate {
            ...
            supports_resume: false,
            supports_json_output: true,
            capabilities: vec!["format", "codegen", "design", "docs"],
            cost_class: HepaAdapterCostClass::Local,
            resource_weight: 1,
            max_concurrency: 4,
        }),
        "custom" => spec(AdapterSpecTemplate {
            ...
            supports_resume: true,
            supports_json_output: true,
            capabilities: vec!["frontend", "design", "backend", "refactor", "docs", "review"],
        ...
        "user-worker" => spec(AdapterSpecTemplate {
            ...
            supports_resume: true,
            supports_json_output: true,
            capabilities: vec!["frontend", "design", "backend", "refactor", "docs"],
            cost_class: HepaAdapterCostClass::PaidCloud,
            resource_weight: 1,
            max_concurrency: 2,
        }),
