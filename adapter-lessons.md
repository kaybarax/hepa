...
- Local Qwen-compatible endpoints can be reachable and still fail live editing
  when they stream only hidden reasoning. HEPA now appends a Pi-only, local
  Qwen no-think prompt boundary so the worker produces content before the
  deterministic stall monitor fires.
